...
function generateModel (path, modelName, modelFields, cb, es6) {
  ft.createDirIfIsNotDefined(path, 'models', function () {
    ...
    var model = ft.loadTemplateSync(es6 ? 'model.es6.js' : 'model.js')
 ...
 * @param {function} cb
 */
function generateRouter (path, modelName, cb, es6) {
  ft.createDirIfIsNotDefined(path, 'routes', function () {
    var router = ft.loadTemplateSync(es6 ? 'router.es6.js' : 'router.js')
    var permissionsName = modelName.replace(/([a-zA-Z])(?=[A-Z])/g, '$1-').toLowerCase()
    router = router.replace(/{controllerName}/g, modelName + 'Controller')
    router = router.replace(/{permissionsName}/g, permissionsName)

    ft.writeFile(path + '/routes/' + modelName + '.js', router, null, cb)
  })
}
 ...
 * @param {function} cb
 */
function generateController (path, modelName, modelFields, cb, es6) {
  ft.createDirIfIsNotDefined(path, 'controllers', function () {
    var controller = ft.loadTemplateSync(es6 ? 'controller.es6.js' : 'controller.js')
    ...
    controller = controller.replace(/{upperName}/g, upperName + 'Model')
